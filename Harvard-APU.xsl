      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          ...
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1081'">
            <xsl:text>हार्वर्ड - APU</xsl:text>
          </xsl:when>
						...
						<xsl:when test="$cAuthorFirstName=1">
							...
							<xsl:choose>
								...
								<xsl:when test ="$cAuthorMiddleName=0">
									<!-- Comment: Add comma only if not last element -->
						...
						<xsl:choose>
							<xsl:when test="$cMonthAccessed!=0">
								<xsl:if test ="$cYearAccessed!=0">
									...
									<!-- COMMENT: Add date suffices -->
									<xsl:choose>
										<xsl:when test="b:DayAccessed=1">
											<xsl:text>st</xsl:text>
										</xsl:when>

										<xsl:when test="b:DayAccessed=2">
											<xsl:text>nd</xsl:text>
										</xsl:when>		
										
										<xsl:when test="b:DayAccessed=3">
